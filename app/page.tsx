...
function YouTubeEmbed({ id, title }: { id: string; title: string }) {
  ...
  return (
    ...
    >
      ...
      />
      {/* Play button overlay */}
      <div className="absolute inset-0 flex items-center justify-center bg-black/30 transition-colors hover:bg-black/10">
        <svg className="h-16 w-16 text-white/90" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
function DiscordButton() {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="group flex h-12 items-center justify-center gap-3 rounded-full border border-white/10 bg-white/5 px-6 font-medium text-white backdrop-blur-sm transition-all duration-300 hover:border-indigo-500/30 hover:bg-indigo-500/10 cursor-pointer"
    >
      <svg className="h-5 w-5 text-white/60 transition-colors group-hover:text-indigo-400" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
...
export default function Home() {
  return (
    <div className="w-full">
      ...
      <section className="relative min-h-[100svh] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex min-h-[100svh] items-center justify-center px-4 py-16">
          <div className="animate-fade-in-up animate-glow-pulse w-full max-w-xl rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl sm:p-10">
            ...
            <p className="animate-fade-in delay-400 mb-6 text-lg font-medium text-white/75">
              Freelance Short Video Editor
            </p>

            <p className="animate-fade-in delay-600 mb-8 leading-relaxed text-white/90">
        ...
        {/* Scroll indicator */}
        <a href="#projects" aria-label="Scroll to projects" className="absolute bottom-8 left-1/2 z-10 -translate-x-1/2 animate-bounce">
          <svg className="h-6 w-6 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
      ...
      {/* ── Projects Section ── */}
      <section id="projects" className="relative w-full overflow-hidden px-4 py-24 section-gradient-projects">
        ...
        <div className="relative mx-auto max-w-5xl">
          <ScrollReveal>
            ...
            <p className="mx-auto mb-16 max-w-md text-center text-white/70">
            ...
            {projects.map((project, i) => (
              <ScrollReveal key={project.id} delay={i * 150}>
                <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 transition-all duration-300 hover:border-white/20 hover:shadow-2xl hover:shadow-white/5">
                  ...
                  <p className="px-4 py-3 text-center text-sm font-medium text-white/70">{project.title}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
        ...
        <div className="relative mx-auto max-w-xl text-center">
          ...
          <ScrollReveal delay={200}>
            <div className="flex flex-col gap-4 sm:flex-row sm:justify-center">
              ...
              >
                <svg className="h-5 w-5 text-white/60 transition-colors group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
